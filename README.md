...
2. 内容分页：在标题下使用 `---` 可将内容分割为多页，每页都会带上标题
...
### 封面图使用
1. 点击顶部工具栏「**封面**」按钮启用，再次点击关闭
2. 封面页自动作为第一张图片插入
3. 点击封面上的标题文字即可编辑；多行输入时**第一行高亮显示**（黄色），其余行为白色
4. 封面底部控件（仅预览可见，导出不包含）：
   - **首行** — 调节第一行字号
   - **正文** — 调节其余行字号
   - **遮罩** — 调节灰色压暗层的透明度（0–100）
   - **位置** — 上下移动标题区域（负值上移，正值下移）
